...
== Compatibility
* 1.5.3
  * Tested with Rails 3.x (but 4 should work as well)
  * Tested with OTRS 3.x

==How to install
    gem 'otrs_connector'

===Configuration Options Rails
...
The API url is the custom API interface file described above.  

=== Configuration Options OTRS
Install the IPhone handle {package}[httpsdoc.otrs.org/3.0/en/html/adminarea-package-manager.html].

The iPhone handle must have the right permissions to access otrs. Edit in the sysconfig option
